...
Interruptor.newAgentTracer({
    ...
    svc:{
        ptrace: {
            onEnter: function(ctx){
                ...
                if(this.cmd === KAPI.CONST.PR_.OPT.PTRACE_ATTACH[0]){
                    ctx.x0 = KAPI.CONST.PR_.OPT.PTRACE_GETEVENTMSG[0];
                }
            },
        },
        prctl: {
            onEnter: function(ctx){
                ...
                if(this.cmd === KAPI.CONST.PR_.OPT.PR_GET_DUMPABLE[0]){
                    console.log("PRCTL GET_DUMPABLE fix PC  ");
                    ctx.pc  = ctx.pc.add(8);
                }
            },
        ...
        newfstatat: {
            onEnter: function(ctx){
                let s = ctx.x1.readCString();
                let i;
                if( (i = s.indexOf("/sbin/su")) >-1) {
                    ctx.x1.writeUtf8String("/sbin/xx");
                    //  setTimeout(()=>{},10000);
                }else if((i = s.indexOf("magisk")) >-1){
                    ctx.x1.add(i).writeUtf8String("xxxxxx");
                }
            }
        },
        faccessat: {
        ...
        openat: {
            onEnter: function (ctx){
                if(ctx.x1.readCString().indexOf('libedb0')>-1){
                    console.log(Thread.backtrace(ctx, Backtracer.ACCURATE).map(DebugSymbol.fromAddress).join('\n'));
                }
            }
        },
        read: {
            ...
            onLeave: function(ctx){
                ...
                /*else if(ctx.pub){
                    const len = ctx.x0.toUInt32();
                    const delta = ctx.x2.toUInt32() ;
                    if(len==0 && PUB_PATCH == 0 && delta > "frida-agent.so".length){
                        PUB_PATCH = 1;
                        ctx.x1 = Memory.allocUtf8String("frida-agent.so");
                        ctx.x0 = delta;
                        console.warn("frida-agent.so injected into /system/etc/public.libraries.txt");
                    }
                    this.pub = false;
                    return;
                }*/

                //console.log(ctx.x1.readCString(ctx.x2.toInt32()));
